...
const finalOrder=`/* AT AI Mobil — V16.9.1F60.94.11 final drawer order + preview chrome guard */
(()=>{
'use strict';
if(window.__AT_FINAL_DRAWER_ORDER_F609411__)return;
window.__AT_FINAL_DRAWER_ORDER_F609411__=true;
const VERSION='FINAL-DRAWER-ORDER-V16.9.1F60.94.11';
...
function installStyle(){
 if($('atFinalDrawerOrderCssF609411'))return;
 const s=document.createElement('style');
 s.id='atFinalDrawerOrderCssF609411';
 s.textContent=`
...
function installPreviewChromeGuard(){
 ...
 if($('atPreviewChromeGuardCssF609411'))return;
 const s=document.createElement('style');
 s.id='atPreviewChromeGuardCssF609411';
 s.textContent=`
...
window.ATFinalDrawerOrderF609411={version:VERSION,apply:applyFinalOrder};
console.info('[AT AI]',VERSION,'active — drawer order is forced visually and physically; preview toolbar hidden on vercel.app.');
})();`;

app+='\n\n'+finalOrder.trim()+'\n';
for(const token of[
 'FINAL-DRAWER-ORDER-V16.9.1F60.94.11',
 'atPreviewChromeGuardCssF609411',
...
console.log('[AT AI] V16.9.1F60.94.11 build complete: drawer order 1-8 enforced and Vercel preview toolbar hidden.');
