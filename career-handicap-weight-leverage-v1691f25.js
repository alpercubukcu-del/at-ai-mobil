/* AT AI Mobil - V16.9.1F25 HANDICAP WEIGHT LEVERAGE
   - Handicap races need a current weight/HP check, not only historical path shape.
   - A horse that won similar conditions at the same HP while carrying more weight gets a capped lift.
   - This is a general rule; it does not name or special-case any horse.
*/
(() => {
'use strict';
if (window.__AT_CAREER_HANDICAP_WEIGHT_LEVERAGE_V1691F25__) return;
window.__AT_CAREER_HANDICAP_WEIGHT_LEVERAGE_V1691F25__ = true;

const VERSION = 'CAREER-HANDICAP-WEIGHT-LEVERAGE-V16.9.1F25';
const SCORE_RULE = 'HANDICAP_SAME_HP_LOWER_WEIGHT_LEVERAGE_V25';
...
function leverageRow(row, target, horse, index) {
  ...
  const days = dayDiff(iso(row), clean(target.date || state?.date || ''));
...
function handicapWeightLeverageScoreF25(item = {}, race = {}) {
  const horse = item?.horse || {};
  const target = { ...normalizeRaceMeta(race), date:clean(state?.date || race?.date || '') };
  ...
  if (topHp !== null && curHp < topHp - 9) {
    return { score:null, used:false, reason:'HP_TOO_LOW_FOR_HANDICAP_FIELD', target, curHp, topHp };
  }
  const rows = careerRows(item?.career || {}).filter(row => finish(row) === 1);
  if (!rows.length) {
    return { score:null, used:false, reason:'NO_WIN_PATH', target, curHp, curWeight, topHp };
  }
  const evidence = rows.map((row, index) => leverageRow(row, target, horse, index));
  ...
  if (!strong.length) {
    return { score:null, used:false, reason:'NO_HANDICAP_WEIGHT_ADVANTAGE', target, curHp, curWeight, topHp, rows:evidence };
  }
  const best = strong[0];
  let score = 90;
  ...
  return {
    ...
    rule:SCORE_RULE,
    version:VERSION,
    note:'Handikap yarışta benzer şart galibiyetine göre bugünkü sıklet avantajı kariyer skorunu destekledi.'
  };
}

function applyHandicapLeverageToCareer(result) {
  ...
  for (const race of result.races) {
    ...
    for (const item of horses) {
      ...
      const leverage = handicapWeightLeverageScoreF25(item, race);
      const currentScore = finite(sim.score);
      const leverageScore = finite(leverage.score);
      const apply = leverageScore !== null && (currentScore === null || leverageScore > currentScore);
...
if (cacheBeforeF25) {
  isValidCareerCache = function(cached) {
    return Boolean(cacheBeforeF25(cached) && hasF25Career(cached));
  };
}
...
function clearStaleCareerF25(reason) {
  try {
    ...
    if (hasF25Career(state.analyses.career)) return false;
...
if (runCareerAnalysisBeforeF25) {
  runCareerAnalysis = async function(selectedRaces, raceValue, ...rest) {
    await runCareerAnalysisBeforeF25(selectedRaces, raceValue, ...rest);
    try {
      if (state?.analyses?.career) {
        const changed = applyHandicapLeverageToCareer(state.analyses.career);
        state.careerHandicapWeightLeverageVersion = VERSION;
        if (typeof save === 'function') save();
      ...
      console.warn('[AT AI] F25 handikap sıklet avantajı uygulanamadı:', e);
    }
  };
}
...
try {
  ...
  if (archive?.hydrateCurrent && !archive.__handicapWeightLeverageF25) {
    ...
    archive.hydrateCurrent = async function(...args) {
      ...
      clearStaleCareerF25('archive-hydrate');
      return out;
    };
    archive.__handicapWeightLeverageF25 = VERSION;
  }
} catch {}

const yearBeforeF25 = typeof yearSimilarityHtml === 'function' ? yearSimilarityHtml : null;
if (yearBeforeF25) {
  yearSimilarityHtml = function(sim) {
    const html = yearBeforeF25(sim);
    ...
    const label = sim?.handicapWeightLeverageApplied
      ? `F25 handikap sıklet avantajı kullanıldı · final %${score ?? '-'} · avantaj %${levScore ?? '-'}`
      : `F25 handikap sıklet avantajı kontrol edildi · avantaj ${levScore === null ? 'yok' : '%' + levScore}`;
...
window.ATCareerHandicapWeightLeverageV1691F25 = {
  version:VERSION,
  scoreRule:SCORE_RULE,
  score:handicapWeightLeverageScoreF25,
  apply:applyHandicapLeverageToCareer
};
console.info('[AT AI]', VERSION, 'aktif - handikapta benzer galibiyete gore kilo avantaji kariyer skorunu destekler.');
})();
