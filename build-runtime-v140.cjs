...
const runtimeFiles = [
  ...
  '__ADAPTIVE_V101_COMPAT__',
  'adaptive-history-v102.js',
  'ticket-models-v11.js',
  '__TICKET_WEIGHT_DISPLAY_COMPAT__',
...
async function main() {
  ...
  const runtimeSourceSet = new Set(runtimeFiles.filter(file => file.endsWith('.js')));
  ...
  const sourceModuleCount = runtimeFiles.filter(file => file.endsWith('.js')).length;
  const runtimeChunks = [
    '/* AT AI SYSTEM — CONSOLIDATED RUNTIME V14.5.1 */',
    `/* Generated at ${new Date().toISOString()} from ${sourceModuleCount} full source modules plus compact compatibility layers. */`,
    'window.__AT_RUNTIME_BUNDLE_V1451__ = true;'
  ];

  for (const file of runtimeFiles) {
    if (file === '__ADAPTIVE_V101_COMPAT__') {
      runtimeChunks.push('\n/* adaptive-history-v101 compatibility only */\n' + adaptiveV101Compat);
      continue;
    }
    if (file === '__TICKET_WEIGHT_DISPLAY_COMPAT__') {
  ...
  const appBundle = [
    '/* AT AI SYSTEM — SINGLE APPLICATION BUNDLE V14.5.1 */',
    `/* app.js kernel + ${sourceModuleCount} full runtime modules + ordered compact compatibility layers */`,
    '\n/* ===== app.js KERNEL ===== */\n' + kernel,
  ...
  console.log(`[AT AI] App V14.5.1 generated: public/${path.basename(APP_OUT)} (${sourceModuleCount} full modules + ordered compact layers)`);
  ...
  console.log('[AT AI] Compatibility order preserved: V10.1 before V10.2; V12.5 before V12.5.1/V12.6.');
