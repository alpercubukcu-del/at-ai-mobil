/* AT AI Mobil — Universal V5 Prototype v16.4
 * Pre-race only. Hedef yarış sonucu/sonrası hiçbir satır skorlamaya alınmaz.
 * Tüm koşu tipleri: ŞARTLI, MAIDEN, HANDİKAP, KV/G, SATIŞ ve diğerleri.
 * Amaç: geçmişten bugüne hedef koşula uyum + güncel form + sınıf/HP + pist/mesafe kanıtı.
 */
(()=>{'use strict';
  const VERSION='UNIVERSAL-V5-V16.4-PROTOTYPE';
  ...
  const weightOf=r=>num(r?.weight??r?.Sıklet??r?.siklet);
  ...
  function features(horse,career,target,field){
    ...
    const evidence=[];const exact=[];const upper=[];let winsNear=0;
    for(const r of rows){
      ...
      const surf=sameTrack?1:.68, fresh=.72+.28*recency(d);
      const value=clamp(base*surf*city*fresh);
      evidence.push({v:value,w:.35+.65*recency(d)});
      if(sameTrack&&ds>=92&&cityOf(r)===tc)exact.push({r,value,d});
      if(cs>=ts&&sameTrack&&ds>=65)upper.push(value);
      if(finish(r)===1&&sameTrack&&ds>=80)winsNear++;
    }
    exact.sort((a,b)=>b.value-a.value);const exactBase=exact.length?weighted(exact.slice(0,3).map((x,i)=>({v:x.value,w:[1,.72,.50][i]}))):0;
    const exactEvidence=exact.length?clamp(exactBase*(.72+.28*Math.min(1,exact.length/3))):0;
    const broadEvidence=evidence.length?weighted(evidence.sort((a,b)=>b.v-a.v).slice(0,4).map((x,i)=>({v:x.v,w:[1,.75,.55,.4][i]}))):0;
    const upperProof=upper.length?Math.max(...upper):0;
    const hp=num(horse?.hp??horse?.HP);const fieldHps=(field||[]).map(x=>num(x?.hp??x?.HP)).filter(Number.isFinite);
    ...
    const wt=num(horse?.weight??horse?.Sıklet);const wts=(field||[]).map(x=>num(x?.weight??x?.Sıklet)).filter(Number.isFinite);
    ...
    let timeScore=50;const exactTimes=[];
    for(const r of rows){if(trackOf(r)!==tt||distOf(r)!==td)continue;const t=timeSec(r?.degree??r?.derece??r?.Derece);if(Number.isFinite(t))exactTimes.push(t);}
    const ownBest=exactTimes.length?Math.min(...exactTimes):null;
    const allBest=[];for(const fh of field||[]){const cid=txt(fh?.id??fh?.horseId??fh?.At_ID);const c=(fh.__careerMap&&fh.__careerMap[cid])||fh?.career||[];for(const r of cutoff(c,target.date)){if(trackOf(r)!==tt||distOf(r)!==td)continue;const t=timeSec(r?.degree??r?.derece??r?.Derece);if(Number.isFinite(t))allBest.push(t);}}
    if(ownBest!==null&&allBest.length){const best=Math.min(...allBest);timeScore=clamp(100-(ownBest-best)*11,35,100);}
    else if(ownBest!==null)timeScore=75;
    return {rows,form,progression,exactEvidence,broadEvidence,upperProof,hpScore,weightScore,handicapEff,timeScore,readiness:readiness(gap),gap,winsNear,careerCount:rows.length,exactCount:exact.length,avgFinish:avgFinish(rows)};
  }
  ...
  function scoreRace({target,horses,careers={},references=[]}={}){
    const hs=Array.isArray(horses)?horses:[];const type=raceType(target?.class);const cleanRefs=(Array.isArray(references)?references:[]).map(r=>({...r,career:cutoff(r?.career||[],r?.referenceDate||r?.date)}));
    const carrier=hs.map(h=>({...h,__careerMap:careers}));
    const rows=carrier.map(h=>{const id=txt(h?.id??h?.horseId??h?.At_ID),name=txt(h?.name??h?.horseName??h?.At_Adı);const c=cutoff(careers[id]||careers[name]||h?.career||[],target.date);const f=features(h,c,target,carrier);const hist=historySimilarity(c,target.date,cleanRefs);const w=classWeights(type);
      ...
      let mode='HISTORY';
      if(f.careerCount===0){mode='DEBUT';const agf=num(h?.agf??h?.AGF);score=35+f.weightScore*.10+f.hpScore*.10+(agf!==null?clamp(agf,0,50)*.20:0);}
      ...
      return {Program_No:num(h?.no??h?.Program_No),At_ID:id,At_Adı:name,Koşu_Tipi:type,Mod:mode,HP:num(h?.hp??h?.HP),Sıklet:num(h?.weight??h?.Sıklet),AGF:num(h?.agf??h?.AGF),V5_SKOR:+clamp(score).toFixed(1),YOL_A:+routeA.toFixed(1),YOL_B:+routeB.toFixed(1),FORM:+f.form.toFixed(1),HEDEF_KANITI:+f.exactEvidence.toFixed(1),BENZER_KANIT:+f.broadEvidence.toFixed(1),UST_SINIF:+f.upperProof.toFixed(1),HP_PUAN:+f.hpScore.toFixed(1),KILO_AVANTAJ:+f.weightScore.toFixed(1),DERECE_KANITI:+f.timeScore.toFixed(1),HAZIRLIK:+f.readiness.toFixed(1),IVME:+f.progression.toFixed(1),TARIHSEL_PROFIL:+hist.toFixed(1),KARIYER:f.careerCount,HEDEF_ORNEK:f.exactCount,VERI_GUVEN:+evidenceCoverage.toFixed(0)};});
    const byScore=rank(rows,'V5_SKOR'),byA=rank(rows,'YOL_A'),byB=rank(rows,'YOL_B');const aMap=new Map(byA.map((r,i)=>[r.At_ID||r.At_Adı,i+1])),bMap=new Map(byB.map((r,i)=>[r.At_ID||r.At_Adı,i+1]));
