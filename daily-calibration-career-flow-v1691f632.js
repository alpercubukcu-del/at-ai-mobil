/* AT AI Mobil — V16.9.1F60.32 Clean Daily Calibration = Career selector flow
   Visible flow:
...
(() => {
'use strict';
if (window.__AT_DAILY_CALIBRATION_CAREER_FLOW_V1691F632__) return;
window.__AT_DAILY_CALIBRATION_CAREER_FLOW_V1691F632__ = true;

const VERSION='DAILY-CALIBRATION-CAREER-FLOW-V16.9.1F60.32';
const DB='at_ai_tjk_annual_archive_v13';
const STORE='races';
const PAGE_ID='dailyCalibrationPageF632';
const DIALOG_ID='dailyCalibrationSelectorF632';
const PAGE_SIZE=100;
...
function selectedRaceNo(){
  const own=Number($('dcalRaceF632')?.value||0);
  if(own)return own;
...
async function resolveRows(rows){
  ...
  return (await mapLimit(rows,3,async row=>{
    ...
    const status=$('dcalSelectorStatusF632');if(status)status.textContent=`Koşu numaraları doğrulanıyor: ${done}/${rows.length}`;
    if(!n)return null;
    if(Number(row.raceNo)!==n){
      row.raceNo=n;
      row.permanentKey=`${row.date}|${row.cityId||''}|${n}`;
      row.resolutionMethod=row.resolutionMethod||'DAILY_CALIBRATION_CAREER_FLOW_F632';
      await putAnnual(row);
    }
    return row;
  })).filter(Boolean);
...
function injectStyle(){
  if($('dailyCalibrationStyleF632'))return;
  const style=document.createElement('style');
  style.id='dailyCalibrationStyleF632';
  style.textContent=`
...
async function refreshResult(){const host=$('dcalResultF632');if(host)host.innerHTML=await resultHtml()}
...
<section class="dc-card">
  ...
  <select id="dcalRaceF632" class="dc-select"></select>
  <div id="dcalTargetF632" class="dc-target"></div>
  <button id="dcalOpenF632" type="button" class="primary dc-open">Eşleşmeleri Gör ve Seç</button>
...
<div id="dcalStatusF632" class="dc-status">Hazır.</div>
<section class="dc-card">
  <div class="dc-title">5 Model Kalibrasyon Sonucu</div>
  <div id="dcalResultF632"></div>
</section>`;
  content.prepend(p);
  $('dcalRaceF632').addEventListener('change',()=>{
    ...
    updateTarget();
    refreshResult();
    const s=$('dcalStatusF632');if(s)s.textContent=`${n}. Koşu seçildi. Eşleşmeleri Gör ve Seç ile geçmiş yarışları belirleyin.`;
  });
  $('dcalOpenF632').onclick=e=>{e.preventDefault();void openSelector()};
  return p;
}

function fillRaceSelect(){
  const select=$('dcalRaceF632');if(!select)return;
...
function updateTarget(){
  const host=$('dcalTargetF632'),c=context();
  if(!host)return;
  host.innerHTML=c
...
function mirrorLegacyStatus(){
  const src=$('xcalStatus'),dst=$('dcalStatusF632');
  if(dst&&src){
...
<header><div class="head"><div><small>AT AI SYSTEM · ${VERSION}</small><h2>Kalibrasyon Eşleşmelerini Seç</h2></div><button id="dcalCloseF632">✕</button></div></header>
<div id="dcalSelectorTargetF632" class="target"></div>
<div class="tools"><div class="tabs">
...
</div><div id="dcalSelectorSummaryF632" class="sub" style="margin-top:7px"></div></div>
<div id="dcalSelectorListF632" class="list"></div>
<footer><div class="actions"><button id="dcalClearF632">Seçimi Temizle</button><button id="dcalApplyF632" class="apply">Seçimi Uygula ve Hesapla</button></div><div id="dcalSelectorStatusF632" class="status"></div></footer>
</div>`;
  document.body.appendChild(d);
  $('dcalCloseF632').onclick=()=>d.close();
  d.querySelectorAll('[data-pick]').forEach(btn=>btn.onclick=()=>{
    ...
    renderMatches();
    $('dcalSelectorStatusF632').textContent=`${btn.textContent} seçildi · ${draft.size} yarış işaretlendi.`;
  });
  $('dcalClearF632').onclick=()=>{
    draft.clear();d.querySelectorAll('[data-pick]').forEach(x=>x.classList.remove('active'));renderMatches();
    $('dcalSelectorStatusF632').textContent='Seçim temizlendi.';
  };
  $('dcalApplyF632').onclick=()=>void applySelection();
  ...
  d.onclick=e=>{if(e.target?.id==='dcalMoreF632'){shown+=PAGE_SIZE;renderMatches()}};
  return d;
}
function renderSummary(){
  const host=$('dcalSelectorSummaryF632');if(!host)return;
  const c={EXACT:0,CONDITION_TWIN:0,RACE_FAMILY:0};matches.forEach(x=>c[x.type]++);
  host.textContent=`${matches.length} eşleşme · Tam ${c.EXACT} · İkiz ${c.CONDITION_TWIN} · Aile ${c.RACE_FAMILY} · ${matches.filter(x=>draft.has(x.row.id)).length} seçili`;
}
function renderMatches(){
  renderSummary();
  const box=$('dcalSelectorListF632');if(!box)return;
...
<div><div class="type">${label(x.type)}</div><div class="sub">uyum %${x.score}</div></div></label>`;
  }).join('')+(matches.length>shown?`<button id="dcalMoreF632" style="width:100%">Daha Fazla Göster · ${fmt(matches.length-shown)} kaldı</button>`:'')
...
async function openSelector(){
  ...
  if(!contextNow){$('dcalSelectorStatusF632').textContent='Önce hedef koşuyu seçin.';return}
  busy=true;
  $('dcalSelectorStatusF632').textContent='Yüklü Yıllık Arşiv taranıyor…';
  try{
    allRows=await readAll();
    matches=allRows.filter(r=>r?.id&&r?.date&&r.date<contextNow.date)
    ...
    draft=new Set([...saved,...(global&&typeof global.values==='function'?[...global]:[])].filter(id=>allowed.has(id)));
    shown=PAGE_SIZE;
    d.querySelectorAll('[data-pick]').forEach(x=>x.classList.remove('active'));
    const years=[...new Set(allRows.map(r=>Number(r?.year||String(r?.date||'').slice(0,4))).filter(Boolean))].sort((a,b)=>a-b);
    $('dcalSelectorTargetF632').innerHTML=`<b>${contextNow.raceNo}. Koşu · ${esc(contextNow.city)} · ${esc(contextNow.date)}</b><br>${esc(contextNow.meta.classRaw)} · ${esc(contextNow.meta.ageGroup)} · ${esc(contextNow.meta.distance)} m ${esc(contextNow.meta.track)}<br>Yüklü arşiv: ${years.length?years[0]+'–'+years.at(-1):'yıl yok'} · ${fmt(allRows.length)} yarış.`;
    renderMatches();
    $('dcalSelectorStatusF632').textContent=matches.length?'Üstten Tümü / Tam / İkiz / Aile seçebilir veya yarışları tek tek işaretleyebilirsin.':'Yüklü yıllarda Tam / İkiz / Aile eşleşmesi yok.';
  }catch(e){
    $('dcalSelectorStatusF632').textContent=e?.message||'Yıllık Arşiv okunamadı.';
  }finally{busy=false}
}
...
async function applySelection(){
  ...
  if(!selected.length){$('dcalSelectorStatusF632').textContent='En az bir eşleşme seçin.';return}
  busy=true;$('dcalApplyF632').disabled=true;
  try{
    const rows=await resolveRows(selected.map(x=>x.row));
    if(!rows.length)throw new Error('Seçilen yarışların koşu numarası doğrulanamadı.');
    ...
    $('dcalSelectorStatusF632').textContent=`${ids.size} geçmiş yarış seçildi · 5 Model kalibrasyonu başlatılıyor…`;
    const status=$('dcalStatusF632');if(status)status.textContent=`${ids.size} geçmiş yarış seçildi · kalibrasyon başlatılıyor…`;
    setTimeout(()=>{
    ...
    $('dcalSelectorStatusF632').textContent=e?.message||'Seçim uygulanamadı.';
  }finally{
    busy=false;$('dcalApplyF632').disabled=false;
  }
}
...
function installRenderPatch(){
  ...
  if(api.__dailyCleanF632)return true;
...
document.addEventListener('change',e=>{
  if(e.target?.matches?.('#xcalRace')&&$('analysisDialog')?.dataset.dailyCalibrationF6018==='1'){
    const own=$('dcalRaceF632');if(own&&[...own.options].some(o=>o.value===e.target.value)){own.value=e.target.value;updateTarget();void refreshResult()}
  }
},true);
...
console.info('[AT AI]',VERSION,'active — clean Daily Calibration page + Career-style Annual Archive selector; F59.4 engine hidden.');
})();