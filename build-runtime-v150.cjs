...
mustReplace(
  ...
  `  const all = await listDateA(date);\n  const city = cleanA(typeof state !== 'undefined' ? state?.city : '');\n  const races = all.filter(r => r.kind === 'race' && cleanA(r.city) === city && recordHasCareerEvidenceA(r)).sort((a,b) => Number(a.raceNo)-Number(b.raceNo));`
);
