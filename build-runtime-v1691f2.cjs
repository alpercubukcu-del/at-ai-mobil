...
for(const token of [
  ...
  'TOP3_EACH_RACE_THEN_BEST_PER_YEAR',
  'EACH_HISTORICAL_RACE_TOP3_PRE_RACE_CAREER',
...
if(!app.includes(".filter(ref => { const f = Number(ref?.finish ?? ref?.rank ?? ref?.sira); return Number.isFinite(f) ? f >= 1 && f <= 3 : true; })")){
  throw new Error('[V16.9.1F2] İlk 3 sabit filtresi bundle içinde bulunamadı.');
}
new Function(app);
