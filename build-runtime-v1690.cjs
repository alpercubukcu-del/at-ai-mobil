...
html=html.replace(/<button data-view="scenario">[^<]*<\/button>/,'<button data-view="scenario">5. Günlük Koşu Kalibrasyonu</button>');
html=html.replace(/<button data-view="career">[^<]*<\/button>/,'<button data-view="career">3. Kariyer Yol Haritası</button>');
html=html.replace(/<button data-view="calibration">[^<]*<\/button>/,'<button data-view="calibration">4. Model Kalibrasyonu</button>');
...
for(const token of ['2. Güncel Analiz','3. Kariyer Yol Haritası','4. Model Kalibrasyonu','5. Günlük Koşu Kalibrasyonu','5. Kupon Oluştur']){
...
console.log('[AT AI] V16.9.0 build tamamlandı: eski historical menü kaldırıldı; mevcut kanonik mobil etiketlerle doğrulandı.');
