...
async function enrichProgramRaceMeta(races) {
  /*
    V5.1:
    Günlük programın koşu şartları artık /api/tjk-program tarafından
    her yarışın kendi race nesnesinde gelir.

    Burada /api/tjk-race-meta çağrısı YOKTUR.
    Böylece 1. koşunun sınıf/yaş/mesafe/pist bilgisinin diğer koşulara
    kopyalanması teknik olarak engellenir.
  */
...
async function loadProgram() {
  ...
  try {
    ...
    status('Koşu şartları TJK günlük programından hazırlanıyor…');
...
function programRaceMeta(race) {
  ...
  const meta = {
    ok: true,
    source: 'TJK Günlük Yarış Programı / TJK-PARSER-V4.3+',
    raceNo: Number(race?.no || 0),
    class:
      race?.class ||
      race?.yaradi1 ||
      '',
  ...
  if (!meta.class) missing.push('sınıf');
  if (!meta.ageGroup) missing.push('yaş grubu');
  if (!meta.distance) missing.push('mesafe');
  if (!meta.track) missing.push('pist');
  ...
  if (missing.length) {
    return {
      ...
      ok: false,
      error:
        `${meta.raceNo}. koşunun günlük program şartları eksik: ` +
        missing.join(', ')
    };
  }
...
function careerRaceAccordionHtml(
  ...
  horses.sort((a, b) => {
    const rawA = a?.galibiyetBenzerligi?.score;
    const rawB = b?.galibiyetBenzerligi?.score;
    const sa =
  ...
  const scoredCount = horses.filter(x => {
    const score = x?.galibiyetBenzerligi?.score;
    return (
      score !== null &&
      score !== undefined &&
      score !== '' &&
      ...
      <div style="padding:2px 10px 10px 10px;">
        ...
        ${horses.length
          ? horses.map((item, index) => {
              const score = item?.galibiyetBenzerligi?.score;
              const hasScore =
                score !== null &&
                score !== undefined &&
                score !== '' &&
                Number.isFinite(Number(score));
              return careerHorseHtml(item, hasScore ? index + 1 : null);
            }).join('')
...
async function runCareerAnalysis(
  ...
  for (const race of selectedRaces) {
    ...
    /*
      KRİTİK V5 DÜZELTMESİ:
      Her koşunun adı/şartı doğrudan günlük programdaki kendi race
      nesnesinden alınır. tjk-race-meta çağrılmaz; böylece 1. koşunun
      şartları diğer koşulara kopyalanamaz.
    */
    ...
    const roadmap = meta?.ok
      ...
      : {
          ...
          error:
            meta?.error ||
            'Günlük programda bu koşunun şartları eksik.'
        };
