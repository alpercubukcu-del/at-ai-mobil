/* AT AI Mobil — F60.86 Sonuç arşivi yenileme sonrası doğrulanmış kalıcı özet */
(() => {
'use strict';
if (window.__AT_F6084_ARCHIVE_PROGRESS_REFRESH__) return;
window.__AT_F6084_ARCHIVE_PROGRESS_REFRESH__ = true;

const VERSION='ARCHIVE-PROGRESS-REFRESH-V16.9.1F60.86';
const DB='at_ai_tjk_annual_results_v1', DAYS='days', RACES='races', META='meta';
const META_RANGE='f6084:last-visible-range', NUMBERING_SOURCE='TJK_RESULTS';
const $=id=>document.getElementById(id);
const clean=v=>String(v??'').replace(/\u00a0/g,' ').replace(/\s+/g,' ').trim();
function iso(v){const s=clean(v);let m=s.match(/^(\d{4})-(\d{1,2})-(\d{1,2})/);if(m)return`${m[1]}-${String(m[2]).padStart(2,'0')}-${String(m[3]).padStart(2,'0')}`;m=s.match(/^(\d{1,2})[./-](\d{1,2})[./-](\d{4})/);return m?`${m[3]}-${String(m[2]).padStart(2,'0')}-${String(m[1]).padStart(2,'0')}`:'';}
...
async function summarize(start,end){
  ...
  const out={days:0,complete:0,verified:0,legacyComplete:0,partial:0,error:0,races:0,postponed:0,minDate:'',maxDate:''};
  try{
    if(db.objectStoreNames.contains(DAYS))await new Promise(resolve=>{const tx=db.transaction(DAYS,'readonly'),os=tx.objectStore(DAYS),idx=os.indexNames.contains('date')?os.index('date'):null,q=idx&&start&&end?idx.openCursor(IDBKeyRange.bound(start,end)):os.openCursor();q.onsuccess=e=>{const c=e.target.result;if(!c)return;const d=c.value,date=iso(d?.date||d?.scheduledDate);if((!start||date>=start)&&(!end||date<=end)){out.days++;if(d?.status==='complete'){out.complete++;if(d?.numberingSource===NUMBERING_SOURCE)out.verified++;else out.legacyComplete++;}else if(Number(d?.raceCount||0)>0)out.partial++;else if(d?.status==='error')out.error++;out.races+=Number(d?.raceCount||0);out.postponed+=Array.isArray(d?.postponedRaceNos)?d.postponedRaceNos.length:0;if(date&&(!out.minDate||date<out.minDate))out.minDate=date;if(date&&(!out.maxDate||date>out.maxDate))out.maxDate=date;}c.continue()};tx.oncomplete=()=>resolve();tx.onerror=tx.onabort=()=>resolve();});
...
function ensureSummaryHost(){let el=$('f6084StoredSummary');if(el)return el;const status=$('f62rStatus');if(!status)return null;el=document.createElement('div');el.id='f6084StoredSummary';el.className='f62-status';el.style.marginTop='8px';status.insertAdjacentElement('afterend',el);return el;}
async function refresh(){
  const r=await range(),s=await summarize(r?.start||'',r?.end||'');if(!s)return;
  const el=ensureSummaryHost();if(!el)return;
  const scope=r?`${r.start.split('-').reverse().join('.')} → ${r.end.split('-').reverse().join('.')}`:'tüm kayıtlar';
  const legacy=s.legacyComplete?` · ${fmt(s.legacyComplete)} eski “tam” toplantı gerçek Koşu No ile doğrulanacak`:'';
  el.textContent=`📦 Telefonda kalıcı sonuç arşivi · ${scope}: ${fmt(s.races)} koşu · ${fmt(s.verified)}/${fmt(s.days)} toplantı TJK gerçek Koşu No ile tam · ${fmt(s.partial)} yarım · ${fmt(s.postponed)} ertelenmiş${legacy}. Kayıtlar IndexedDB'de korunur.`;
  const st=$('f62rStatus');if(st&&!clean(st.textContent))st.textContent='Kayıtlı sonuç arşivi telefondan yeniden okundu. Eski “tam” kayıtlar gerçek Koşu No ile doğrulanmadan atlanmaz.';
}
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',schedule,{once:true});else schedule();setTimeout(schedule,2000);
window.ATF6084ArchiveProgress={version:VERSION,refresh,summarize};
console.info('[AT AI]',VERSION,'aktif — eski tam toplantılar ile TJK gerçek Koşu No doğrulanmış toplantılar ayrı sayılır.');
})();
