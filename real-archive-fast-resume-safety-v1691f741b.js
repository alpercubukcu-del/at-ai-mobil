...
async function resetPartial(){
  ...
  await new Promise(resolve=>{
    try{
      ...
      q.onsuccess=()=>{const c=q.result;if(!c)return;const v=c.value;if(v?.status==='partial'&&String(v?.version||'').includes(MARK)){v.nextPage=0;v.resumeSafety='REFETCH_UNFINISHED_PARALLEL_PAGES';v.updatedAt=new Date().toISOString();c.update(v)}c.continue()};
      tx.oncomplete=()=>resolve();tx.onerror=tx.onabort=()=>resolve();
    }catch{resolve()}
  });
  try{db.close()}catch{}
}
async function resetSelectedPartial(){
  const a=Number(document.getElementById('rrFastYearFromF60943121')?.value),b=Number(document.getElementById('rrFastYearToF60943121')?.value);
  if(!a||!b)return;
  const db=await open();
  if(!db||!db.objectStoreNames.contains(STORE)){try{db?.close()}catch{}return}
  await new Promise(resolve=>{
    try{
      ...
      for(let y=Math.min(a,b);y<=Math.max(a,b);y++){
        const q=os.get(`year:${y}:scan`);
        q.onsuccess=()=>{const v=q.result;if(v?.status==='partial'&&String(v?.version||'').includes(MARK)){v.nextPage=0;v.resumeSafety='STOP_REFETCH_FROM_PAGE_ZERO';v.updatedAt=new Date().toISOString();os.put(v)}};
      }
      tx.oncomplete=()=>resolve();tx.onerror=tx.onabort=()=>resolve();
    }catch{resolve()}
  });
