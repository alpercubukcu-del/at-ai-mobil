...
function resultKey(result){return `${String(result?.date||'')}|${String(result?.city||'')}`}
function selectedKey(){try{return `${String(state?.date||'')}|${String(state?.city||'')}`}catch{return '|'}}
function contextMatches(result){
  if(!result?.races?.length)return false;
  try{
    if(typeof gCurrentValidV1657==='function')return !!gCurrentValidV1657(result);
  }catch{}
  const key=resultKey(result),sel=selectedKey();
  return !!result?.date&&!!result?.city&&key===sel&&(!result?.type||result.type==='current');
}
function cardHtml(result){
  ...
  return `<div id="degreeSpeedShadowF6090" data-ui-guard="${VERSION}" data-analysis-date="${esc(result?.date||'')}" data-analysis-city="${esc(result?.city||'')}" style="margin:0 0 12px;border:1px solid rgba(126,226,168,.25);border-radius:12px;padding:10px;background:rgba(126,226,168,.04)"><b>DERECE-HIZ · GÖLGE MODEL</b><div style="font-size:11px;opacity:.75;margin-top:4px">${esc(result?.city||'')} · ${esc(result?.date||'')} · Tahmini derece, derece aralığı ve orijin desteği. Ana sıralamayı şimdilik değiştirmez.</div><div style="margin-top:7px;font-size:11px;line-height:1.6">${esc(source)}${Number.isFinite(w.temperature)?` · ${esc(w.temperature)}°C`:''}${Number.isFinite(w.humidity)?` · Nem %${esc(w.humidity)}`:''}${Number.isFinite(w.pressure)?` · ${esc(w.pressure)} hPa`:''}${w.sky?` · ${esc(w.sky)}`:''}${w.wind?` · ${esc(w.wind)}`:''}</div>${resilient}${races}</div>`;
}
async function resilientEnrich(expectedResult=null){
  const engine=window.ATDegreeSpeedF6090,live=expectedResult||currentResult(),expectedKey=resultKey(live);
  if(!engine?.enrichCurrent||!contextMatches(live)||expectedKey!==selectedKey())return false;
  try{
    await engine.enrichCurrent();
    if(expectedKey!==selectedKey()||!contextMatches(currentResult()))return false;
  ...
  if(!contextMatches(result)||resultKey(result)!==expectedKey)return false;
  ...
  if(expectedKey!==selectedKey()||!contextMatches(result))return false;
...
async function ensureCard(result=null){
  const content=document.getElementById('analysisContent');if(!content)return;
  let candidate=result&&contextMatches(result)?result:currentResult();
  const existing=document.getElementById('degreeSpeedShadowF6090');
  if(!contextMatches(candidate)){
    if(existing)existing.remove();
    return;
  }
  const expectedKey=resultKey(candidate);
  const existingKey=existing?`${String(existing.dataset.analysisDate||'')}|${String(existing.dataset.analysisCity||'')}`:'';
  if(existing&&existingKey!==expectedKey)existing.remove();
  if(!candidate.degreeSpeed&&window.ATDegreeSpeedF6090?.enrichCurrent&&!enriching){
    enriching=true;try{await resilientEnrich(candidate)}finally{enriching=false}
  }
  candidate=currentResult();
  if(!contextMatches(candidate)||resultKey(candidate)!==expectedKey||expectedKey!==selectedKey()){
    document.getElementById('degreeSpeedShadowF6090')?.remove();
    return;
  }
  const fresh=document.getElementById('degreeSpeedShadowF6090');
  const freshKey=fresh?`${String(fresh.dataset.analysisDate||'')}|${String(fresh.dataset.analysisCity||'')}`:'';
  if(fresh&&freshKey===expectedKey)return;
  if(fresh)fresh.remove();
  if(candidate?.degreeSpeed)content.insertAdjacentHTML('afterbegin',cardHtml(candidate));
}
function schedule(result=null){clearTimeout(timer);timer=setTimeout(()=>{void ensureCard(result)},20)}
function syncSelection(){
  const live=currentResult(),card=document.getElementById('degreeSpeedShadowF6090');
  if(!contextMatches(live)){
    if(card)card.remove();
    return;
  }
  const expected=resultKey(live),shown=card?`${String(card.dataset.analysisDate||'')}|${String(card.dataset.analysisCity||'')}`:'';
  if(card&&shown!==expected)card.remove();
  if(!document.getElementById('degreeSpeedShadowF6090'))schedule(live);
}
try{
  if(typeof gRenderCurrentV1657==='function'){
    const priorRender=gRenderCurrentV1657;
    gRenderCurrentV1657=function(result,raceFilter='all'){const out=priorRender(result,raceFilter);if(contextMatches(result))schedule(result);else document.getElementById('degreeSpeedShadowF6090')?.remove();return out};
  }
...
window.ATDegreeSpeedUiGuardF6094={version:VERSION,ensure:ensureCard,resilientEnrich,contextMatches};
console.info('[AT AI]',VERSION,'aktif — derece kartı tarih/şehir izolasyonu ve tek-koşu hata yalıtımıyla korunur.');
})();