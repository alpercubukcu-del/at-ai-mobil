...
for(const token of[
  'CURRENT-ANALYSIS-MOBILE-SCROLL-V16.9.1F60.64',
  "data-f63-scroll-mode='custom'",
...
for(const token of[
  ...
  'data-f63-scroll-mode',
  selectedRefFirst
])if(!app.includes(token))throw new Error('[F60.64] Verification failed: '+token);
new Function(app);
fs.writeFileSync(APP,app,'utf8');
let html=fs.readFileSync(INDEX,'utf8').replace(/\/at-ai-app-v142\.js\?v=\d+/,'/at-ai-app-v142.js?v=169270');
fs.writeFileSync(INDEX,html,'utf8');
if(!html.includes('/at-ai-app-v142.js?v=169270'))throw new Error('[F60.64] Cache bust failed.');
console.log('[AT AI] V16.9.1F60.64 build complete: Güncel Analiz > Özel Android panel is its own vertical scroll host; sticky Reset/Calculate preserved; Standart/Sonuçlar continue to use normal analysis content.');
