...
for(const f of [BASE,ROUTING_PATCH,MENU_PATCH,NUMBER_PATCH,COMPACT_PATCH,BET_DROPDOWN_PATCH,WINNER_SHARED_PATCH]) if(!fs.existsSync(f)) throw new Error(`[V16.8.7] Eksik dosya: ${path.basename(f)}`);
execFileSync(process.execPath,[BASE],{cwd:ROOT,stdio:'inherit'});
for(const f of [APP,INDEX]) if(!fs.existsSync(f)) throw new Error(`[V16.8.7] Build sonrası eksik dosya: ${path.relative(ROOT,f)}`);
...
for(const token of [
  ...
  if(!app.includes(token)) throw new Error(`[V16.8.7] production bundle doğrulaması başarısız: ${token}`);
}
if(app.indexOf('FIVE-MODEL-SHARED-CACHE-V16.8.7')>app.indexOf('COUPON-DECISION-GATE-V16.7.1')) {
  throw new Error('[V16.8.7] ortak 5 Model cache kupon karar kapısından sonra yüklenmiş.');
}
if(app.indexOf('WINNER-PATH-SHARED-CACHE-V16.8.7')<app.indexOf('WINNER-PATH-DEDICATED-V16.6.9')) {
  throw new Error('[V16.8.7] ortak Kazanan Yolu cache dedicated kör test katmanından önce yüklenmiş.');
}
if(app.includes("attributeFilter:['class','style','aria-hidden']")) {
  throw new Error('[V16.8.7] eski style MutationObserver freeze yolu bundle içinde kaldı.');
}
if(app.includes("document.body.style.setProperty('position','fixed','important')")) {
  throw new Error('[V16.8.7] eski body position:fixed kupon kilidi bundle içinde kaldı.');
}
new Function(app);
...
for(const token of ['id="couponMenuBtn"','id="couponCenterDialog"','id="couponSetupV1681"','id="couponAuditHostV1681"','id="tickets"']) {
  if(!html.includes(token)) throw new Error(`[V16.8.7] hamburger kupon HTML eksik: ${token}`);
}
const mainMatch=html.match(/<main>([\s\S]*?)<\/main>/i);
if(!mainMatch) throw new Error('[V16.8.7] <main> bulunamadı.');
for(const forbidden of ['buildAllBtn','betTypes','couponSetupV1681','id="tickets"','KUPON MERKEZİ','OLUŞTURULAN KUPON','manualBetSheetV117']) {
  if(mainMatch[1].includes(forbidden)) throw new Error(`[V16.8.7] ana sayfada kupon kalıntısı var: ${forbidden}`);
}
html=html.replace(/\/at-ai-app-v142\.js\?v=\d+/,'/at-ai-app-v142.js?v=16870');
fs.writeFileSync(INDEX,html,'utf8');

console.log('[AT AI] V16.8.7 build tamamlandı: 5 Model ve Kazanan Yolu kompakt session cache ile sayfa yenilemesinde de tekrar kullanılır; cache v=16870.');
