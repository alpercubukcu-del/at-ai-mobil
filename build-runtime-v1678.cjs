...
for(const f of [BASE,ROUTING_PATCH,MENU_PATCH,NUMBER_PATCH,COMPACT_PATCH,BET_DROPDOWN_PATCH,WINNER_SHARED_PATCH,MODEL_ROADMAP_CACHE_PATCH]) if(!fs.existsSync(f)) throw new Error(`[V16.8.8] Eksik dosya: ${path.basename(f)}`);
...
// En sona eklenir: eski fetchModelRoadmapV11 override'larının tamamını kesin olarak cache endpoint'ine yönlendirir.
...
for(const token of [
  ...
  '5 Model tarihsel yol haritası CDN cache üzerinden alınır',
...
console.log('[AT AI] V16.8.8 build tamamlandı: 5 Model/Kazanan Yolu session reuse + tarihsel 5 Model roadmap CDN cache; cache v=16880.');
