...
function dateValue(id){const e=$(id);if(!e)return'';const v=String(e.value||'').trim();if(/^\d{4}-\d{2}-\d{2}$/.test(v))return v;if(e.valueAsDate&&!Number.isNaN(e.valueAsDate.getTime())){const d=e.valueAsDate;return `${d.getUTCFullYear()}-${String(d.getUTCMonth()+1).padStart(2,'0')}-${String(d.getUTCDate()).padStart(2,'0')}`}const m=v.match(/^(\d{1,2})[.\/-](\d{1,2})[.\/-](\d{4})$/);return m?`${m[3]}-${m[2].padStart(2,'0')}-${m[1].padStart(2,'0')}`:''}
function realDates(){const a=dateValue('rrDateFromF6093'),b=dateValue('rrDateToF6093');return a&&b&&a<=b?[a,b]:null}
...
function upgradePanel(){
  ...
  try{
    ...
    const legacyBtn=[...root.querySelectorAll('button')].find(b=>/Pist\s*\/\s*Bakım\s*\/\s*Hava Arşivi/i.test((b.textContent||'').trim()));if(legacyBtn)legacyBtn.remove();
