...
function years(){const a=Number($('rrYearFromF6093')?.value),b=Number($('rrYearToF6093')?.value),y=currentYear();if(!Number.isInteger(a)||!Number.isInteger(b)||a<1800||b<1800||a>y||b>y)return null;return[Math.min(a,b),Math.max(a,b)]}
...
async function runRealYears(){
  const yr=years();if(!yr){setRealStatus(`Yıl aralığı 1800-${currentYear()} arasında olmalı.`,0);return}
  ...
  const[a,b]=yr;setRealBusy(true);setRealStatus(`${a}-${b} gerçekleşmiş yarışları Koşu Sorgulama ile indeksleyip sonuçları telefona indiriyor…`,0);
  try{await api.syncYears(a,b);await refreshRealMeta();await applyArchiveToAnalyses(false);setRealStatus(`${a}-${b} gerçek yarış arşivi tamamlandı. Sonuçlar analiz kaynağına bağlandı.`,100)}
  catch(e){setRealStatus('Gerçek yarış arşivi indirme hatası: '+(e?.message||e),0)}
...
function realSectionHtml(y){return `<section id="realRaceArchiveSectionF6093" class="tmr418-card tmr418-real"><div class="tmr418-kicker">1 · GERÇEK YARIŞ SONUÇLARI</div><h3>Gerçek Yarış Arşivi · Koşu Sorgulama</h3><div class="tmr418-note"><b>Veri hattı:</b> Koşu Sorgulama → Gerçekleşmiş Yarış İndeksi → Günlük Yarış Sonuçları → Telefonda Gerçek Yarış Arşivi → Analizler.</div><div class="tmr416-grid tmr418-grid"><label>Başlangıç yılı<input id="rrYearFromF6093" type="number" inputmode="numeric" min="1800" max="${y}" step="1" value="${y}"></label><label>Bitiş yılı<input id="rrYearToF6093" type="number" inputmode="numeric" min="1800" max="${y}" step="1" value="${y}"></label></div><button id="rrDownloadF6093" class="primary" type="button">Seçili Yılları İndir / Kaldığı Yerden Devam Et</button><button id="rrUpdateF6093" type="button">Bugüne Kadar Eksikleri Güncelle</button><button id="rrApplyAnalysisF609418" type="button">Arşivi Analizlere Yeniden Bağla</button><div class="tmr416-bar"><i id="rrBarF6093"></i></div><div id="rrStatusF6093" class="tmr416-status">Hazır.</div><div id="rrMetaF6093" class="tmr416-status">Gerçekleşmiş yarış arşivi okunuyor…</div></section>`}
