...
function inputIso(id){const e=$(id);if(!e)return'';if(e.valueAsDate&&!Number.isNaN(e.valueAsDate.getTime()))return iso(e.valueAsDate);const v=String(e.value||'').trim();if(/^\\d{4}-\\d{2}-\\d{2}$/.test(v))return v;const m=v.match(/^(\\d{1,2})[.\\/-](\\d{1,2})[.\\/-](\\d{4})$/);return m?`${m[3]}-${m[2].padStart(2,'0')}-${m[1].padStart(2,'0')}`:''}
function realDates(){const a=inputIso('rrDateFromF6093'),b=inputIso('rrDateToF6093');return a&&b&&a<=b?[a,b]:null}
