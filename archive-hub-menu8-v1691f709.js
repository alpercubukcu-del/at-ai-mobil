...
async function runRealYears(){
  const dr=dates();if(!dr){setRealStatus('Başlangıç ve bitiş tarihini seçin.',0);return}
  ...
  const[a,b]=dr;setRealBusy(true);setRealStatus(`${a} - ${b} gerçekleşmiş yarışları Koşu Sorgulama ile indeksleyip sonuçları telefona indiriyor…`,0);
  try{await api.syncRange(a,b);await refreshRealMeta();await applyArchiveToAnalyses(false);setRealStatus(`${a} - ${b} gerçek yarış arşivi tamamlandı. Sonuçlar analiz kaynağına bağlandı.`,100)}
