...
function realDates(){const a=$('rrDateFromF6093')?.value,b=$('rrDateToF6093')?.value;return /^\d{4}-\d{2}-\d{2}$/.test(a)&&/^\d{4}-\d{2}-\d{2}$/.test(b)&&a<=b?[a,b]:null}
...
async function runRealYears(){
  ...
  try{while(cur<=b){setRealStatus(`Gerçek yarış arşivi · ${cur} · ${done+1}/${total}`,Math.round(done/total*100));if(api.syncRange)await api.syncRange(cur,cur);else if(api.syncRecent)await api.syncRecent({start:cur,end:cur,force:true});else await api.syncYears(Number(cur.slice(0,4)),Number(cur.slice(0,4)));saveProg('at_ai_real_archive_daily_progress_v1',{lastDate:cur,nextDate:addDay(cur),from:a,to:b,updatedAt:new Date().toISOString()});cur=addDay(cur);done++;renderDailyProgress()}await refreshRealMeta();await applyArchiveToAnalyses(false);setRealStatus(`${a} → ${b} gerçek yarış arşivi tamamlandı.`,100)}catch(e){setRealStatus('Gerçek yarış arşivi indirme hatası: '+(e?.message||e),0)}finally{setRealBusy(false)}}
...
async function runTrackDays(){const a=$('tmDateFromF609418')?.value,b=$('tmDateToF609418')?.value;if(!/^\\d{4}-\\d{2}-\\d{2}$/.test(a)||!/^\\d{4}-\\d{2}-\\d{2}$/.test(b)||a>b)return setRealStatus('Pist/Bakım/Hava için geçerli tarih aralığı seçin.',0);const api=window.ATTrackMaintenanceV1;if(!api?.syncRange)return setRealStatus('Pist/Bakım/Hava motoru hazır değil.',0);let cur=a;try{while(cur<=b){await api.syncRange(cur,cur,{loadReports:true,label:`Pist/Bakım/Hava ${cur}`});saveProg('at_ai_track_daily_progress_v1',{lastDate:cur,nextDate:addDay(cur),from:a,to:b,updatedAt:new Date().toISOString()});renderDailyProgress();cur=addDay(cur)}}catch(e){setRealStatus('Pist/Bakım/Hava indirme hatası: '+(e?.message||e),0)}}
