...
const VERSION='TJK-RACE-QUERY-V1.1-F60.93';
...
function parseRows(html){
  ...
  $('tr').each((_,tr)=>{
    ...
    const date=iso(field(['Tarih'],0)),city=field(['SehirAdi','Sehir'],1),raceNo=Number(dec(field(['KosuNo','KNo','Kosu'],2))||0);
...
function query(start,end,page){const qs=new URLSearchParams();qs.set('QueryParameter_Tarih_Start',display(start));qs.set('QueryParameter_Tarih_End',display(end));if(Number(page)>0)qs.set('PageNumber',String(page));return qs.toString()}
function urls(start,end,page){const q=query(start,end,page);return Number(page)>0?[`${TJK}${DATA}?${q}`,`${TJK}${FILTER}?${q}`,`${TJK}${PAGE}?${q}`]:[`${TJK}${DATA}?${q}`,`${TJK}${FILTER}?${q}`,`${TJK}${PAGE}?${q}`]}
...
export default async function handler(req,res){
  res.setHeader('Access-Control-Allow-Origin','*');res.setHeader('Cache-Control','public, max-age=0, s-maxage=21600, stale-while-revalidate=86400');
  try{
    ...
    return res.status(200).json({ok:true,version:VERSION,start,end,page,total:d.total,rows,rawRowCount:d.rows.length,filterMatched:rows.length>0||d.rows.length===0,sourceUrl:d.sourceUrl});
