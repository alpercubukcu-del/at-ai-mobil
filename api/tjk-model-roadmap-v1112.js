const VERSION = 'TJK-MODEL-ROADMAP-V11.12';
...
function candidateKey(x = {}) { return [clean(x.date), clean(x.city).toLocaleUpperCase('tr-TR'), Number(x.raceNo || 0)].join('|'); }
...
function careerEnvelope(career = {}) {
  ...
  return {
    ok:Boolean(career.ok), error:career?.ok ? null : (career?.error || 'Kariyer alınamadı.'),
    cutoffExclusive:career.before || null,
    winsBefore:wins, winsBeforeCount:wins.length,
    top5Before:top5, top5BeforeCount:top5.length,
    preparationPathBefore:prep, preparationPathBeforeCount:prep.length,
    analysisMode:career.analysisMode || null, audit:career.audit || null, version:career.version || null
  };
}
...
function normalizeCandidate(raw = {}, year = null) {
  ...
  return {
    date:clean(raw.date || raw.isoDate || ''), city:clean(raw.city || ''), raceNo:Number(raw.raceNo || 0),
    sourceYear:Number(raw.sourceYear || year || String(raw.date || '').slice(0,4)) || null,
    anchorDate:clean(raw.anchorDate || ''), calendarDayDifference:Number(raw.calendarDayDifference ?? 999),
    referenceType:type, referenceLabel:raw.referenceLabel || typeLabel(type),
    transferabilityScore:Number(raw.transferabilityScore ?? raw.raceConditionSimilarity ?? 100),
    transferabilityTier:raw.transferabilityTier || null, transferabilityColor:raw.transferabilityColor || null,
    explanation:raw.explanation || '', distance:Number(raw.distance || 0) || null,
    track:raw.track || '', class:raw.class || '', ageGroup:raw.ageGroup || ''
  };
}
async function buildHistoricalRace(baseUrl, candidate) {
  ...
  try {
    ...
    output.condition = {
      class:history.class || candidate.class || '', ageGroup:history.ageGroup || candidate.ageGroup || '',
      distance:history.distance || candidate.distance || '', track:history.track || candidate.track || '', raw:history.conditionRaw || null
    };
    const top3 = (Array.isArray(history.top3) ? history.top3 : [])
      .map(normalizeHorse).filter(h => h.finish >= 1 && h.finish <= 3).sort((a,b) => a.finish - b.finish).slice(0,3);
...
function chooseBestPerType(yearResult = {}) {
  ...
  for (const type of MODEL_TYPES) {
    const candidates = matches.map(x => normalizeCandidate(x, yearResult.year))
      .filter(x => x.referenceType === type && x.date && x.city && x.raceNo)
      .sort((a,b) => Number(b.transferabilityScore || 0) - Number(a.transferabilityScore || 0) ||
        Number(a.calendarDayDifference || 999) - Number(b.calendarDayDifference || 999) || String(b.date).localeCompare(String(a.date)));
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const sourceUrl = new URL('/api/tjk-adaptive-roadmap-v103', baseUrl);
    ...
    const source = await fetchJson(sourceUrl.toString(), 155000, 1);
    if (!source?.ok) throw new Error(source?.error || 'V10.3 tarihsel tarama alınamadı.');

    const prebuilt = new Map();
    for (const race of Array.isArray(source.historicalRaces) ? source.historicalRaces : []) prebuilt.set(candidateKey(race), race);
    ...
    const built = await mapLimit(unique, RACE_CONCURRENCY, async candidate => {
      ...
      if (existing && normalizeType(existing.referenceType) === candidate.referenceType && Array.isArray(existing.top3)) {
        return { ...existing, referenceType:candidate.referenceType, referenceLabel:candidate.referenceLabel,
          transferabilityScore:candidate.transferabilityScore, sourceYear:candidate.sourceYear,
    ...
    for (const type of MODEL_TYPES) models[type].sort((a,b) => Number(b.sourceYear || 0) - Number(a.sourceYear || 0));

    res.setHeader('Cache-Control', 's-maxage=600, stale-while-revalidate=1800');
    return res.status(200).json({
      ok:true, version:VERSION, sourceVersion:source.version || null, target:source.target || null,
      classAlias:source.classAlias || null, classMatching:source.classMatching || null,
      rules:{
        yearWindow:'target month/day ±45 days, every historical year separately', independentModels:true,
        modelTypes:MODEL_TYPES, candidateRule:'best candidate of each reference type per year among accepted matches',
        top3CareerFreeze:'historical horse career only before historical race date',
        genericClassMatching:'original TJK class first; canonical alias fallback only when original scan has zero matches'
      },
      models, counts:Object.fromEntries(MODEL_TYPES.map(type => [type, models[type].length])),
      diagnostics:{
        sourceYears:Array.isArray(source.yearResults) ? source.yearResults.length : 0,
        candidateCount:unique.length, builtCount:built.length, durationMs:Date.now()-startedAt,
        classMatching:source.classMatching || null,
        warning:'Missing model type is no data, never zero and never program-number ranking.'
      }
    });
  } catch (e) {
    return res.status(500).json({ ok:false, version:VERSION, error:e?.message || 'V11.12 model yol haritası hazırlanamadı.', durationMs:Date.now()-startedAt });
  }
}
