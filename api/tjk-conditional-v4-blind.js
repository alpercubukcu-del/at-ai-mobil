...
const VERSION='CONDITIONAL-V4-BLIND-RUNNER-V1';
const APP_BASE='https://at-ai-mobil.vercel.app';
const FROZEN_FILE='conditional-v4-prototype-v163.js';
...
function rowsFrom(root){
  ...
  function visit(v,depth=0){
    ...
    if(Array.isArray(v)){
      ...
      for(const x of v.slice(0,5))visit(x,depth+1);
      return;
    }
  ...
  const a=scored[0]?.a||[];
  const z=new Map();for(const x of a){const d=dateOf(x);if(!d)continue;const k=clean(x?.uniqueKey)||[d,clean(x?.city??x?.sehir??x?.Şehir),clean(x?.distance??x?.mesafe??x?.Mesafe),finish(x),clean(x?.class??x?.raceClass??x?.classRaw??x?.Koşu_Sınıfı)].join('|');z.set(k,x);}return [...z.values()].sort((a,b)=>dateOf(a).localeCompare(dateOf(b)));
}
function loadFrozenV4(){
  const file=path.join(process.cwd(),FROZEN_FILE);const src=fs.readFileSync(file,'utf8');
  const sandbox={module:{exports:{}},exports:{},console};vm.runInNewContext(src,sandbox,{filename:FROZEN_FILE,timeout:2000});
  const api=sandbox.module.exports;if(!api?.scoreRace)throw new Error('Dondurulmuş V4 modülü yüklenemedi.');return api;
}
...
function buildRefs(roadmap,targetDate){
  const map=new Map();for(const type of ['EXACT','CONDITION_TWIN','RACE_FAMILY']) for(const r of (roadmap?.models?.[type]||[])){
    ...
    for(const q of (r?.top3||[])){
      if(finish(q)!==1)continue;const id=clean(q?.horseId),name=clean(q?.horseName);const key=[rd,clean(r?.city),r?.raceNo,id||name].join('|');
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(!/ŞARTLI|SARTLI/i.test(target.class))throw new Error('Seçilen yarış Şartlı değil.');
    ...
    const careers={};const coverage=[];for(const x of careerResults){careers[String(x.h.id)]=x.rows;const future=x.rows.filter(r=>dateOf(r)&&dateOf(r)>=date).length;coverage.push({At_ID:String(x.h.id),At_Adı:x.h.name,Kariyer_Satırı:x.rows.length,Son_Tarih:maxDate(x.rows),FutureLeak:future,Coverage:x.d?.coverageStatus??x.d?.audit?.coverageStatus??x.d?.coverage?.status??null,Valid:x.d?.valid??x.d?.audit?.valid??null});}
    ...
    const roadmap=await getJSON(`${APP_BASE}/api/tjk-model-roadmap-v11?${p.toString()}`,210000);
    const refs=buildRefs(roadmap,date);
    ...
    const api=loadFrozenV4();const scored=api.scoreRace({target,horses,careers,references:refs});
    const currentLeaks=coverage.reduce((s,x)=>s+x.FutureLeak,0),refLeaks=refAudit.reduce((s,x)=>s+x.Leak,0);
    return res.status(200).json({ok:true,runnerVersion:VERSION,frozenModelVersion:api.VERSION,target,resultApiCalled:false,resultDataLoaded:false,leakAudit:{currentFutureLeakCount:currentLeaks,referenceFutureLeakCount:refLeaks,passed:currentLeaks===0&&refLeaks===0},coverage,referenceWinners:refAudit,policy:scored.policy,rows:scored.rows});
