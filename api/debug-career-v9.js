...
function parseRows(html) {
  ...
  for (const tr of trs) {
    ...
    rows.push({
      isoDate:iso,
      city:clean(r.city),
      distance:parseIntValue(r.distance) || 0,
      track:clean(r.track),
      finish:parseIntValue(r.finish) || 0,
...
function key(r) {
  return [r.isoDate, upper(r.city), r.distance, r.finish, r.degree].join('|');
}
...
export default async function handler(req, res) {
  try {
    ...
    const years = meta.selectYears.length ? meta.selectYears : Object.keys(meta.summaryYears).map(Number).sort((a,b) => b-a);
    ...
    return res.status(200).json({
      ok:true,
      version:'DEBUG-CAREER-V9-1',
      horseId,
      first:{
        careerTotal:meta.total,
        parsedRows:firstRows.length,
        firstDate:firstRows[0]?.isoDate || null,
        lastDate:firstRows[firstRows.length - 1]?.isoDate || null,
        summaryYears:meta.summaryYears,
        selectYears:meta.selectYears
      },
      yearDiagnostics,
      union:{
        rows:unionRows.length,
        wins:unionRows.filter(r => r.finish === 1).length,
        firstDate:unionRows[0]?.isoDate || null,
        lastDate:unionRows[unionRows.length - 1]?.isoDate || null,
        complete:meta.total !== null && unionRows.length === meta.total
      },
      winRows:unionRows.filter(r => r.finish === 1)
    });
  } catch (e) {
    return res.status(500).json({ ok:false, version:'DEBUG-CAREER-V9-1', error:e?.message || String(e) });
  }
}
